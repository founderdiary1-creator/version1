...
import { useUpdateStoryMutation, useAdminStoryQuery } from '@/hooks/useStories';
...
export default function EditArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: article, isLoading: fetching } = useAdminStoryQuery(id);
  ...
  useEffect(() => {
    if (article) {
      ...
      if (article.summary_points && article.summary_points.length > 0) {
        setSummaryPoints(article.summary_points);
      }
      ...
      // Initialize Blocks: Support both new structure and legacy content auto-upgrade
      if (article.content_blocks && article.content_blocks.length > 0) {
        setBlocks(article.content_blocks.map((b: any) => ({
        ...
        // Auto-upgrade legacy HTML string into the first block
        setBlocks([{
        ...
        setBlocks([{ id: 'initial-block', subheading: '', content: '', imageFile: null, imagePreview: '', image_caption: '' }]);
      }
    }
  }, [article]);
  ...
  function updateBlock(blockId: string, field: keyof ContentBlockState, value: any) {
    setBlocks(prev => prev.map(b => b.id === blockId ? { ...b, [field]: value } : b));
  }

  function handleBlockImage(blockId: string, e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (file) {
      updateBlock(blockId, 'imageFile', file);
      updateBlock(blockId, 'imagePreview', URL.createObjectURL(file));
    }
  }
 ...
 // --- Submission ---
  async function handleSubmit(status: 'draft' | 'published') {
    ...
    if (!blocks.some(b => b.content.trim() || b.imageFile || b.imagePreview)) {
      return toast.error('Please add some content to the article');
    }

    setLoading(true);
    const loadingToast = toast.loading(status === 'published' ? 'Updating published intelligence...' : 'Saving draft updates...');
    ...
    try {
      ...
      // 3. Save to Database
      toast.loading('Saving updates to database...', { id: loadingToast });
      ...
      toast.success(status === 'published' ? 'Article updated and published!' : 'Draft updates saved!', { id: loadingToast });
      router.push('/admin');
    } catch (err) {
      console.error(err);
      toast.error(err instanceof Error ? err.message : 'Failed to update article', { id: loadingToast });
    } finally {
      setLoading(false);
    }
  }
